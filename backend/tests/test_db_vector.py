...
class TestDBVectorManager(unittest.TestCase):
    ...
    def test_add_and_fetch_one(self) -> None:
        ...
        fetch_response = self.manager.fetch_one("doc-001")
    ...
    def test_fetch_all(self) -> None:
        ...
        self.manager.add(ids=["a"], documents=["Alpha doc"], metadatas=[{"tag": "a"}])
        self.manager.add(ids=["b"], documents=["Beta doc"],  metadatas=[{"tag": "b"}])
        self.manager.add(ids=["c"], documents=["Gamma doc"], metadatas=[{"tag": "c"}])

        response = self.manager.fetch_all()
        self.assertTrue(response["success"])
        data = response["data"]
    ...
    def test_fetch_all_with_where_filter(self) -> None:
        """fetch_all with a metadata where-filter returns only matching docs."""
        self.manager.add(ids=["x"], documents=["X doc"], metadatas=[{"env": "prod"}])
        self.manager.add(ids=["y"], documents=["Y doc"], metadatas=[{"env": "dev"}])

        response = self.manager.fetch_all(where={"env": "prod"})
        self.assertTrue(response["success"])
    ...
    def test_update(self) -> None:
        ...
        update_response = self.manager.update(
            ids=["upd-1"],
        ...
        fetch_response = self.manager.fetch_one("upd-1")
        self.assertTrue(fetch_response["success"])
        doc = fetch_response["data"]
    ...
    def test_delete(self) -> None:
        ...
        self.manager.add(ids=["del-1"], documents=["To be deleted."])

        delete_response = self.manager.delete(ids=["del-1"])
        self.assertTrue(delete_response["success"])
        self.assertEqual(delete_response["data"]["count"], 1)

        fetch_response = self.manager.fetch_one("del-1")
    ...
    def test_add_duplicate_id_fails(self) -> None:
        ...
        self.manager.add(ids=["dup-1"], documents=["First insert."])

        response = self.manager.add(ids=["dup-1"], documents=["Duplicate insert."])
        self.assertFalse(response["success"])
        self.assertIsNone(response["data"])
        self.assertIsNotNone(response["message"])   # Error message must be present

    def test_fetch_one_not_found(self) -> None:
        """Fetching a non-existent ID returns success=True with data=None."""
        response = self.manager.fetch_one("ghost-id-99999")
        self.assertTrue(response["success"])
        self.assertIsNone(response["data"])
        self.assertIn("not found", response["message"].lower())

    def test_update_requires_payload(self) -> None:
        """update() with no documents/metadatas/embeddings returns success=False."""
        self.manager.add(ids=["empty-upd"], documents=["Some text."])
        response = self.manager.update(ids=["empty-upd"])   # nothing to update
        self.assertFalse(response["success"])
        self.assertIn("requires at least one of", response["message"])

    def test_delete_empty_ids_fails(self) -> None:
        """delete() called with an empty list returns success=False."""
        response = self.manager.delete(ids=[])
        self.assertFalse(response["success"])
    ...
    def test_collection_exists(self) -> None:
        ...
        self.manager.add(ids=["ce-1"], documents=["Existence check doc."])
        response = self.manager.collection_exists()
        self.assertTrue(response["success"])
